    ...
    if (targetSlot) {
      ...
      if (movingCarIndex !== targetIndex) {
        ...
        if (
          targetCar && 
          draggedCar && 
          draggedCar.level > 0 && // Проверяем, что уровень перетаскиваемой машинки не 0
          targetCar.level > 0 && // Проверяем, что уровень целевой машинки не 0
          draggedCar.level === targetCar.level
        ) {
          // Объединяем машинки
          ownedCars[targetIndex].level++; 
          ownedCars[movingCarIndex] = { level: 0, name: `Car ${movingCarIndex + 1}` }; // Сбрасываем исходный слот до 0 уровня
        } else {
          // Меняем машинки местами
        ...
        try {
          ...
          // Обновляем отображение и пересчитываем доход в любом случае
          displayCars(); 
          updateEarnRate();
        }
      }
